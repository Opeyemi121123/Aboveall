define(["jquery", "underscore", "backbone"], function($, _, Backbone) {
    ...
    var url = 'mongodb+srv://anthonycampbell736:zeoHIVVVAkH4BbyM@cluster0.6m9ra.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0'; // Your MongoDB connection string
    var dbName = 'Aboveall'; // Your actual database name
    var collectionName = 'Theone'; // Your actual collection name
    ...
    var AppView = Backbone.View.extend({
        ...
        render: function() {
            ...
            MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, function(err, client) {
                if (err) {
                    console.error(err);
                    self.$el.html("Database connection error");
                    return;
                }
                ...
                collection.find({}).toArray(function(err, docs) {
                    if (err) {
                        console.error(err);
                        self.$el.html("Error fetching data");
                        return;
                    }
                    ...
                    docs.forEach(function(doc) {
                        dataHtml += "<li>" + JSON.stringify(doc) + "</li>";
                    });
                    dataHtml += "</ul>";

                    self.$el.html(dataHtml);
                    client.close();
                });
            });
        }
